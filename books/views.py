...
logger = logging.getLogger(__name__)  # 로그 출력을 위한 logger 객체 생성
...
def book_list(request):
    ...
    # logger.info(f"도서 목록 조회: {elapsed_time}초 소요")  # 로그 출력(추가)
...
def loan_create(request, book_id):
    ...
    return render(
        request, "books/loan_form.html", {"form": form, "book": book}
    )  # 대출 폼 렌더링
...
def loan_overdue_list(request):
    ...
    return render(
        request, "books/loan_list.html", {"loans": overdue_loans, "show_overdue": True}
    )
...
def reservation_create(request, book_id):
    ...
    if request.method == "POST":  # POST 요청인 경우
        ...
        if form.is_valid():
            ...
            reservation.expiry_date = timezone.now() + timedelta(
                days=1
            )  # 1일간 예약 유효
            reservation.save()
...
def reservation_cancel(request, reservation_id):
    ...
    if (
        request.user != reservation.user and not request.user.is_librarian()
    ):  # 본인의 예약이거나 사서만 취소 가능
        messages.error(request, "권한이 없습니다.")
